...
RUN apk add --update --no-cache chromium procps
ENV PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=0
RUN yarn playwright-install
RUN rm /root/.cache/ms-playwright/chromium-930007/chrome-linux/chrome && \
    ln -s /usr/bin/chromium-browser /root/.cache/ms-playwright/chromium-930007/chrome-linux/chrome

# Run integration tests (Browser)
RUN VSCODE_REMOTE_SERVER_PATH="$(pwd)/../vscode-reh-web-linux-alpine" \
